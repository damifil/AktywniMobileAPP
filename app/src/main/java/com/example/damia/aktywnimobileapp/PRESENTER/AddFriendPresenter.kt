...
class AddFriendPresenter(fragment: AddFriendFragment) {
    ...
    fun init() {
        ...
        if(fragment.param1.equals("")) {
        arr = JSONArray(sharedPreferenceApi.getString(fragment.context!!, EnumChoice.friendList))
        }
        else
        {
            arr = JSONArray(sharedPreferenceApi.getString(fragment.context!!, EnumChoice.usersEvent))
        }
